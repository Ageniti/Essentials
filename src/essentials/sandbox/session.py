...
if TYPE_CHECKING:
    ...
    from essentials.sandbox.docker_backend import DockerSandboxSession
...
async def start_docker_sandbox(
    ...
    """Start a Docker sandbox session for the current Essentials session."""
    global _active_session  # noqa: PLW0603

    from essentials.sandbox.docker_backend import DockerSandboxSession, get_docker_availability

    availability = get_docker_availability(settings)
    if not availability.available:
        if settings.sandbox.fail_if_unavailable:
            from essentials.sandbox.adapter import SandboxUnavailableError
