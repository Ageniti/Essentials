...
from essentials.api.client import AnthropicApiClient, SupportsStreamingMessages
...
from essentials.commands import (
    CommandContext,
    CommandResult,
...
from essentials.coordinator.coordinator_mode import is_coordinator_mode
from essentials.engine import QueryEngine
from essentials.engine.messages import (
    ConversationMessage,
    ToolResultBlock,
    ToolUseBlock,
    sanitize_conversation_messages,
)
from essentials.engine.query import MaxTurnsExceeded
from essentials.engine.stream_events import StreamEvent
from essentials.hooks import HookEvent, HookExecutionContext, HookExecutor, load_hook_registry
from essentials.hooks.hot_reload import HookReloader
from essentials.mcp.client import McpClientManager
from essentials.mcp.config import load_mcp_server_configs
from essentials.permissions import PermissionChecker
from essentials.plugins import load_plugins
from essentials.prompts import build_runtime_system_prompt
from essentials.state import AppState, AppStateStore
from essentials.services.session_backend import DEFAULT_SESSION_BACKEND, SessionBackend
from essentials.tools import ToolRegistry, create_default_tool_registry
from essentials.keybindings import load_keybindings
...
class RuntimeBundle:
    ...
    def current_settings(self):
        ...
        We persist most settings to disk (``~/.essentials/settings.json``), but
        ...
        """
        return load_settings(config_path=self.settings_path).merge_cli_overrides(**self.settings_overrides)
    ...
    def hook_summary(self) -> str:
        ...
        if self.bare:
            return "Bare mode: hooks disabled."
    ...
    def plugin_summary(self) -> str:
        ...
        if self.bare:
            return "Bare mode: plugins disabled."
    ...
    def _safe_resolve_auth():
        try:
            ...
            print(
                ...
                "  Run `essentials auth login` to set up authentication, or set the\n"
...
def _apply_appended_system_prompt(settings, append_system_prompt: str | None):
    """Append CLI text to the resolved system prompt while preserving overrides."""
    if not append_system_prompt:
        return settings
    appended = append_system_prompt.strip()
    if not appended:
        return settings
    existing = (settings.system_prompt or "").strip()
    merged = f"{existing}\n\n{appended}".strip() if existing else appended
    return settings.model_copy(update={"system_prompt": merged})
...
async def build_runtime(
    *,
    ...
    settings_input: str | Path | None = None,
    ...
    """Build the shared runtime for an Essentials session."""
    ...
    settings = load_settings(config_path=settings_path).merge_cli_overrides(**settings_overrides)
    settings = _apply_appended_system_prompt(settings, append_system_prompt)
    settings_overrides["system_prompt"] = settings.system_prompt
    cwd = str(Path(cwd).expanduser().resolve()) if cwd else str(Path.cwd())
    normalized_skill_dirs = (
        ()
        if bare
        else tuple(str(Path(path).expanduser().resolve()) for path in (extra_skill_dirs or ()))
    )
    normalized_plugin_roots = (
        ()
        if bare
        else tuple(str(Path(path).expanduser().resolve()) for path in (extra_plugin_roots or ()))
    )
    ...
    mcp_manager = McpClientManager({} if bare else load_mcp_server_configs(settings, plugins))
    ...
    if bare:
        hook_registry = load_hook_registry(settings.model_copy(update={"hooks": {}}), [])
    else:
    ...
    restored_metadata = {
        "permission_mode": settings.permission.mode.value,
        "session_mode": "coordinator" if is_coordinator_mode() else "default",
        "read_file_state": [],
        "invoked_skills": [],
        "async_agent_state": [],
        "async_agent_tasks": [],
    ...
    if settings.sandbox.enabled and settings.sandbox.backend == "docker":
        from essentials.sandbox.session import start_docker_sandbox
...
async def close_runtime(bundle: RuntimeBundle) -> None:
    ...
    from essentials.sandbox.session import stop_docker_sandbox
...
async def handle_line(
    ...
    *,
    ...
    if not bundle.external_api_client:
        current_settings = bundle.current_settings()
        bundle.hook_executor.update_registry(
            load_hook_registry(
                current_settings if not bundle.bare else current_settings.model_copy(update={"hooks": {}}),
...
async def _render_command_result(
    ...
    if result.replay_messages and render_event is not None:
        ...
        from essentials.engine.stream_events import AssistantTextDelta, AssistantTurnComplete
