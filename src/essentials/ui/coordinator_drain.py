...
from essentials.coordinator.coordinator_mode import (
    TaskNotification,
...
def format_completed_task_notifications(completed: list[dict[str, object]]) -> str:
    ...
    for entry in completed:
        ...
        task = manager.get_task(task_id)
        if task is None:
            task_status = str(entry.get("status") or "missing")
            output = str(entry.get("result") or "").strip()
            summary = (
                str(entry.get("summary") or "").strip()
                or f'Agent "{entry.get("description") or agent_id}" is no longer available in this session'
            )
        else:
...
async def submit_follow_up(
    ...
    *,
    ...
    from essentials.ui.runtime import _format_pending_tool_results
