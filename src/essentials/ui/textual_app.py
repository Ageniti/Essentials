"""Default Textual terminal UI for Essentials."""
...
from essentials.api.client import SupportsStreamingMessages
from essentials.coordinator.coordinator_mode import is_coordinator_mode
from essentials.engine.stream_events import (
    AssistantTextDelta,
...
from essentials.tasks import get_task_manager
from essentials.ui.coordinator_drain import drain_coordinator_async_agents
from essentials.ui.runtime import build_runtime, close_runtime, handle_line, start_runtime
...
class EssentialsTerminalApp(App[None]):
    ...
    def compose(self) -> ComposeResult:
        ...
        with Horizontal(id="main-row"):
            with Vertical(id="transcript-column"):
                ...
                yield Input(placeholder="Ask Essentials or enter a /command", id="composer")
    ...
    async def on_mount(self) -> None:
        self._bundle = await build_runtime(
            ...
            settings_input=self._config.settings_input,
            ...
            enforce_max_turns=self._config.max_turns is not None,
            permission_mode=self._config.permission_mode,
            allowed_tools=self._config.allowed_tools,
            denied_tools=self._config.disallowed_tools,
            bare=self._config.bare,
        )
        await start_runtime(self._bundle)
        self.query_one("#composer", Input).focus()
        self._refresh_sidebars(force=True)
        if is_coordinator_mode() and self._config.restore_tool_metadata:
            self.call_later(lambda: asyncio.create_task(self._drain_restored_async_agents()))
    ...
    async def _drain_restored_async_agents(self) -> None:
...
async def run_textual_app(
    *,
    ...
    """Launch the built-in Textual interface."""
    app = EssentialsTerminalApp(
        prompt=prompt,
        cwd=cwd,
        model=model,
        max_turns=max_turns,
