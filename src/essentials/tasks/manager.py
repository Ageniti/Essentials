...
from essentials.config.paths import get_tasks_dir
from essentials.tasks.types import TaskRecord, TaskStatus, TaskType
from essentials.utils.shell import create_shell_subprocess

log = logging.getLogger(__name__)
_TASK_RESTART_NOTICE = "[Essentials] Agent task restarted; prior interactive context was not preserved.\n"
...
class BackgroundTaskManager:
        ...
        *,
        ...
        if command is None and argv is None:
            ...
            try:
                ...
                raise ValueError(
                    "Local agent tasks require configured provider credentials or an explicit command/argv override"
                ) from exc
            argv = [
                "python",
                "-m",
    ...
    def set_task_metadata(self, task_id: str, **metadata: object) -> TaskRecord:
        """Update persisted task metadata fields."""
        task = self._require_task(task_id)
        for key, value in metadata.items():
            if value is None:
                task.metadata.pop(key, None)
            else:
                task.metadata[str(key)] = str(value)
            ...
            if process.returncode is None:
                ...
                if task is not None and task.status == "running":
                    ...
                    task.metadata.setdefault(
                        "status_note",
                        "Previous session ended before task completion; interactive context was not preserved.",
                    )
                    self._save_task(task)
    ...
    def _save_task(self, task: TaskRecord) -> None:
        ...
        self._task_state_path(task.id).write_text(
            json.dumps(payload, ensure_ascii=False, indent=2) + "\n",
            encoding="utf-8",
        )

    def _load_persisted_tasks(self) -> None:
        for path in sorted(get_tasks_dir().glob("*.json")):
            try:
                ...
                task = TaskRecord(
                    ...
                    argv=list(raw.get("argv") or []) or None,
                )
            except Exception:
                log.exception("Failed to load persisted task state from %s", path)
                continue
            ...
            self._output_locks[task.id] = asyncio.Lock()
            self._input_locks[task.id] = asyncio.Lock()
            self._save_task(task)
