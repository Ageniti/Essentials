...
from essentials.coordinator.agent_definitions import (
    AGENT_COLORS,
    EFFORT_LEVELS,
    MEMORY_SCOPES,
    PERMISSION_MODES,
...
from essentials.plugins.schemas import PluginManifest
from essentials.plugins.types import LoadedPlugin, PluginCommandDefinition
from essentials.skills.loader import _parse_skill_metadata
from essentials.skills.types import SkillDefinition
...
def get_project_plugins_dir(cwd: str | Path) -> Path:
    ...
    path = Path(cwd).resolve() / ".essentials" / "plugins"
    path.mkdir(parents=True, exist_ok=True)
    return path
...
def _load_plugin_mcp(path: Path) -> dict[str, object]:
    ...
    if not path.exists():
        return {}
    from essentials.mcp.types import McpJsonConfig
...
def _load_plugin_tools(path: Path, manifest: PluginManifest) -> list:
    ...
    from essentials.tools.base import BaseTool
