"""Agent definition loading system for Essentials."""
...
from essentials.config.paths import get_config_dir
...
_SHARED_AGENT_PREFIX = (
    "You are an agent for Essentials, a focused local coding assistant CLI. "
...
_EXPLORE_SYSTEM_PROMPT = """You are a file search specialist for Essentials. You excel at thoroughly navigating and exploring codebases.
...
_PLAN_SYSTEM_PROMPT = """You are a software architect and planning specialist for Essentials. Your role is to explore the codebase and design implementation plans.
...
1. Read the project's README and local instruction files for build/test commands and conventions. Check package.json / Makefile / pyproject.toml for script names. If the implementer pointed you to a plan or spec file, read it — that's the success criteria.
...
- **Intentional**: do project instructions / comments / commit message explain this as deliberate?
...
_STATUSLINE_SYSTEM_PROMPT = """You are a status line setup agent for Essentials. Your job is to create or update the statusLine command in the user's Essentials settings.
...
2. For longer commands, you can save a new file in the user's ~/.essentials directory, e.g.:
   - ~/.essentials/statusline-command.sh and reference that file in the settings.

3. Update the user's ~/.essentials/settings.json with:
   {
     "statusLine": {
       "type": "command",
...
4. If ~/.essentials/settings.json is a symlink, update the target file instead.
...
_CLAUDE_CODE_GUIDE_SYSTEM_PROMPT = """You are the Essentials guide agent. Your primary responsibility is helping users understand and use Essentials, its configuration surface, and the provider APIs it can talk to effectively.

**Your expertise spans three domains:**

1. **Essentials** (the CLI tool): Installation, configuration, hooks, skills, MCP servers, keyboard shortcuts, IDE integrations, settings, and workflows.

2. **Provider integration patterns**: OpenAI-compatible and Anthropic-compatible configuration, auth setup, and runtime behaviour.

3. **Project-local behaviour**: Local prompts, hooks, plugins, settings, and workflow conventions in this repository.

**Documentation sources:**

- **Local repository files**: Prefer local source of truth first for questions about Essentials behaviour:
  - README
  - settings and config models
  - hooks, skills, MCP, plugins, and CLI implementation

- **Provider documentation**: Use WebFetch/WebSearch only when the answer depends on external provider APIs or official third-party docs.

**Approach:**
1. Determine which domain the user's question falls into
2. Read the relevant local files first
3. Use WebFetch/WebSearch only when local code or docs are insufficient
4. Fetch the most relevant external pages only when needed
5. Provide clear, actionable guidance grounded in the repo and official docs
...
- Reference exact documentation URLs in your responses when external docs are used
- Help users discover features by proactively suggesting related commands, shortcuts, or capabilities
- When you cannot find an answer or the feature doesn't exist, say so plainly and point to the relevant local limitation
...
_BUILTIN_AGENTS: list[AgentDefinition] = [
    ...
    AgentDefinition(
        ...
        description="Use this agent to configure the user's Essentials status line setting.",
        tools=["Read", "Edit"],
    ...
    AgentDefinition(
        ...
        description=(
            ...
            '"How do I...") about: (1) Essentials (the CLI tool) - features, hooks, slash '
            "commands, MCP servers, settings, IDE integrations, keyboard shortcuts; "
            "(2) provider-compatible API setup; (3) local configuration and workflow details. "
            "**IMPORTANT:** Before spawning a "
...
def normalize_permission_mode(raw: Any) -> str | None:
    """Validate agent permission modes against the simplified runtime modes."""
    ...
    * ``subagent_type`` — routing key (Python-specific, defaults to name)
    ...
    for path in sorted(directory.glob("*.md")):
        try:
            ...
            permission_mode = normalize_permission_mode(perm_raw)
            if permission_mode is None and perm_raw is not None:
...
def get_all_agent_definitions() -> list[AgentDefinition]:
    ...
    1. Built-in agents
    2. User agents (~/.essentials/agents/)
    ...
    try:
        from essentials.plugins.loader import load_plugins  # noqa: PLC0415
        from essentials.config.settings import load_settings  # noqa: PLC0415
