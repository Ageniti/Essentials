...
from essentials.coordinator.agent_definitions import get_agent_definition
from essentials.coordinator.coordinator_mode import get_team_registry
from essentials.hooks import HookEvent
from essentials.swarm.registry import get_backend_registry
from essentials.swarm.types import TeammateSpawnConfig
...
class AgentTool(BaseTool):
    ...
    async def execute(self, arguments: AgentToolInput, context: ToolExecutionContext) -> ToolResult:
        ...
        # Use the only supported backend so spawned agents are registered in
        # BackgroundTaskManager and remain pollable by task tools.
