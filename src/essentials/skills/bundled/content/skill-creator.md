...
name: skill-creator
description: >
  Create, improve, and verify Essentials skills. Use this whenever the user
  asks to create a new skill, convert a workflow into a skill, update an
  existing SKILL.md, add skills for Essentials or a plugin, design skill trigger behavior,
  or test whether a skill loads and works correctly.
---

# skill-creator

Create or improve Essentials skills in the directory-based `SKILL.md` format.
...
- make a skill available to Essentials or a plugin
- debug why the `skill` tool cannot find or load a skill
- test trigger wording, skill metadata, or skill behavior

## Essentials skill locations

Choose the target deliberately:

- Built-in Essentials skills live in `src/essentials/skills/bundled/content/*.md`.
- User skills for `essentials` live in `~/.essentials/skills/<skill-dir>/SKILL.md`.
- Plugin skills live in `<plugin-root>/skills/<skill-dir>/SKILL.md`.

Essentials user and plugin skills use a directory layout. Do not create flat
...
2. Inspect existing examples.
   ...
   Essentials, confirm whether the target is bundled, user, or
   plugin-provided.
