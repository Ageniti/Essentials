...
from essentials.swarm.spawn_utils import (
...
class SubprocessBackend:
    ...
    Uses the existing :class:`~essentials.tasks.manager.BackgroundTaskManager`
    ...
    async def spawn(self, config: TeammateSpawnConfig) -> SpawnResult:
        ...
        if command is None:
            ...
            if (
                ...
                argv = [teammate_cmd, "-m", "essentials", "--task-worker"] + flags
            else:
    ...
    async def send_message(self, agent_id: str, message: TeammateMessage) -> None:
        ...
        if task_id is None:
            ...
            task_id = self._agent_tasks.get(agent_id)
        if task_id is None:
            raise ValueError(f"No known subprocess task for agent {agent_id!r}")
    ...
    def _restore_agent_mappings(self) -> None:
        manager = get_task_manager()
        for task in manager.list_tasks():
            agent_id = str(task.metadata.get("agent_id") or "").strip()
            if agent_id:
                self._agent_tasks[agent_id] = task.id
