"""Settings model and loading logic for Essentials.

Settings are resolved with the following precedence (highest first):
1. CLI arguments
2. Environment variables (OPENAI_API_KEY, ANTHROPIC_API_KEY, ESSENTIALS_MODEL, etc.)
3. Config file (~/.essentials/settings.json)
4. Defaults
"""
...
from essentials.hooks.schemas import HookDefinition
from essentials.mcp.types import McpServerConfig
from essentials.permissions.modes import PermissionMode
from essentials.utils.file_lock import exclusive_file_lock
from essentials.utils.fs import atomic_write_text
...
class DockerSandboxSettings(BaseModel):
    ...
    image: str = "essentials-sandbox:latest"
...
class Settings(BaseModel):
    """Main settings model for Essentials."""
    ...
    def resolve_api_key(self) -> str:
        ...
        raise ValueError(
            ...
            "~/.essentials/settings.json"
        )

    def resolve_auth(self) -> ResolvedAuth:
        """Resolve auth for the current provider, including external CLI-backed auth."""
        profile_name, profile = self.resolve_profile()
        ...
        from essentials.auth.storage import load_credential
    ...
    def merge_cli_overrides(self, **overrides: Any) -> Settings:
        ...
        denied_tools = updates.pop("denied_tools", updates.pop("disallowed_tools", None))
        ...
        if permission_updates:
            updates["permission"] = self.permission.model_copy(update=permission_updates)
...
def _apply_env_overrides(settings: Settings) -> Settings:
    ...
    ``OPENAI_BASE_URL``, ``OPENAI_MODEL``) only apply when the active profile does *not*
    explicitly configure the corresponding field. ``ESSENTIALS_*`` env vars
    always override.
    """
    ...
    # --- model ---
    essentials_model = os.environ.get("ESSENTIALS_MODEL")
    if essentials_model:
        updates["model"] = strip_ansi_escape_sequences(essentials_model)
        ...
        if anthropic_model:
            ...
            if openai_model:
                updates["model"] = strip_ansi_escape_sequences(openai_model)

    # --- base_url ---
    essentials_base = os.environ.get("ESSENTIALS_BASE_URL")
    if essentials_base:
        updates["base_url"] = essentials_base
    ...
    max_tokens = os.environ.get("ESSENTIALS_MAX_TOKENS")
    if max_tokens:
        updates["max_tokens"] = int(max_tokens)

    timeout = os.environ.get("ESSENTIALS_TIMEOUT")
    if timeout:
        updates["timeout"] = float(timeout)

    max_turns = os.environ.get("ESSENTIALS_MAX_TURNS")
    if max_turns:
        updates["max_turns"] = int(max_turns)

    context_window_tokens = os.environ.get("ESSENTIALS_CONTEXT_WINDOW_TOKENS")
    if context_window_tokens:
        updates["context_window_tokens"] = int(context_window_tokens)

    auto_compact_threshold_tokens = os.environ.get("ESSENTIALS_AUTO_COMPACT_THRESHOLD_TOKENS")
    ...
    api_format = os.environ.get("ESSENTIALS_API_FORMAT")
    if api_format:
        updates["api_format"] = api_format

    provider = os.environ.get("ESSENTIALS_PROVIDER")
    if provider:
        updates["provider"] = provider

    sandbox_enabled = os.environ.get("ESSENTIALS_SANDBOX_ENABLED")
    sandbox_fail = os.environ.get("ESSENTIALS_SANDBOX_FAIL_IF_UNAVAILABLE")
    sandbox_backend = os.environ.get("ESSENTIALS_SANDBOX_BACKEND")
    sandbox_docker_image = os.environ.get("ESSENTIALS_SANDBOX_DOCKER_IMAGE")
...
def resolve_settings_path(settings_input: str | None) -> Path | None:
    """Resolve a CLI settings input into a concrete settings file path."""
    if settings_input is None:
        return None

    raw = settings_input.strip()
    if not raw:
        return None
    ...
    raise ValueError(f"--settings path does not exist: {candidate}")
...
def load_settings(config_path: Path | None = None) -> Settings:
    ...
    if config_path is None:
        from essentials.config.paths import get_config_file_path
