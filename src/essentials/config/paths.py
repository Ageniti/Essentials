"""Path resolution for Essentials configuration and data directories.

Follows XDG-like conventions with ~/.essentials/ as the default base directory.
"""
...
def get_config_dir() -> Path:
    ...
    Resolution order:
    1. ESSENTIALS_CONFIG_DIR environment variable
    2. ~/.essentials/
    """
    env_dir = os.environ.get("ESSENTIALS_CONFIG_DIR")
    if env_dir:
...
def get_config_file_path() -> Path:
    """Return the path to the main settings file (~/.essentials/settings.json)."""
...
def get_data_dir() -> Path:
    ...
    Resolution order:
    1. ESSENTIALS_DATA_DIR environment variable
    2. ~/.essentials/data/
    """
    env_dir = os.environ.get("ESSENTIALS_DATA_DIR")
    if env_dir:
...
def get_logs_dir() -> Path:
    ...
    Resolution order:
    1. ESSENTIALS_LOGS_DIR environment variable
    2. ~/.essentials/logs/
    """
    env_dir = os.environ.get("ESSENTIALS_LOGS_DIR")
    if env_dir:
...
def get_project_config_dir(cwd: str | Path) -> Path:
    """Return the per-project .essentials directory."""
    project_dir = Path(cwd).resolve() / ".essentials"
...
def get_project_context_file(cwd: str | Path) -> Path:
    """Return the optional per-project context file."""
